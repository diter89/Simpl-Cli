...
from cores.wallet_cache_handler import load_from_cache
...
def analyze_selected_assets_with_llm(assets: list):
    if not assets:
        return
...
def run_interactive_session(address: str):
    """
    Run interactive explorer session loop for specific address.
    This is the main function that will be called by main.py.
    """
    ...
    while True:
        selected_assets = inquirer.fuzzy(
            message="Select assets to analyze (type to search, space to select, enter to confirm):",
        ...
        if want_analysis:
            analyze_selected_assets_with_llm(selected_assets)
