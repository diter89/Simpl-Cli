...
class EnhancedSearchPersona:
    def __init__(self):
        self.trusted_domains = {
            ...
            'reddit.com': 0.4, 'quora.com': 0.4  # Lower trust for user-generated content
        }
        self.search_cache = {}
...
- For the latest info, prioritize results from {now.year}
"""

    def _classify_query_type(self, query: str) -> str:
        """Enhanced query classification"""
        patterns = {
            'code': [r'\b(code|example|implementation|tutorial|how to)\b',
                    r'\b(python|javascript|react|api|github)\b'],
            'temporal': [r'\b(price|latest|now|current|recent|news|update|20\d{2})\b',
                        r'\b(funding|investment|launch|release)\b'],
            'definition': [r'\b(what is|definition|explain)\b'],
            'comparison': [r'\b(vs|versus|compared to|compare)\b'],
        }
    ...
    def _generate_smart_queries(self, base_query: str, query_type: str, max_queries: int = 4) -> List[str]:
        ...
        fallback_queries = {
            ...
            'temporal': [f"{base_query} {current_year}", f"{base_query} latest news",
                        f"{base_query} {current_month_year}", f"{base_query} recent updates"],
...
- Include temporal markers for recent info
...
Return only a JSON array of strings."""
    ...
    def _enhanced_search_with_validation(self, query: str, query_type: str) -> List[SearchResult]:
        """Enhanced search with result validation and scoring"""
        ...
        try:
            ...
            search_response = brave_search(query, limit=12)
    ...
    def _synthesize_results(self, all_results: List[SearchResult], user_query: str, query_type: str) -> str:
        ...
        if not all_results:
            return "Sorry, no relevant information was found. Please try different keywords."
