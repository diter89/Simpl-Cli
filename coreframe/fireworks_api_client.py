...
CONFIG = {
    "fireworks": {
        ...
        "default_model": "accounts/sentientfoundation/models/dobby-unhinged-llama-3-3-70b-new", #"accounts/sentientfoundation-serverless/models/dobby-mini-unhinged-plus-llama-3-1-8b",
        "max_tokens": 16000  
    },
    "huggingface": {
...
# Default value for 'layanan' parameter changed from "fireworks" to "huggingface".
def generate_response(messages: list, stream: bool = False, model: str = None, temperature: float = 0.7, response_format: dict = None, layanan: str = "fireworks"):
    ...
    if model is None:
        model_to_use = cfg["default_model"]
    else:
        model_to_use = model
    ...
    try:
        # --- Streaming Mode (Original Logic Intact) ---
        if stream:
        ...
        # --- Non-Streaming Mode (Original Logic Intact) ---
        else:
    ...
    # --- Error Handling (Original Logic Intact) ---
